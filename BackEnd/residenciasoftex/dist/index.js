...
app.post('/login', (req, res) => {
    ...
    connection.query('SELECT * FROM clientes WHERE email = ?', [email], async (err, results) => {
        ...
        const token = jsonwebtoken_1.default.sign({ id: user.id, email: user.email, isAdmin: user.isAdmin }, JWT_SECRET, { expiresIn: '1h' });
        res.json({ token, id: user.id });
    });
});
// Rota para buscar todos os clientes
app.get('/clientes', authenticateToken, autenticarAdmin, (req, res) => {
...
app.get('/clientes/:id', authenticateToken, autenticarAdmin, (req, res) => {
    const { id } = req.params;
...
app.delete('/clientes/:id', authenticateToken, autenticarAdmin, (req, res) => {
    const { id } = req.params;
...
app.post('/salas', authenticateToken, autenticarAdmin, (req, res) => {
    const data = req.body;
    if (!validarSala(data)) {
...
/*app.post('/salas', authenticateToken, autenticarAdmin, (req: Request, res: Response) => {
  ...
  const novaSala = { ...data, data_cadastro };

  connection.query('INSERT INTO salas SET ?', novaSala, (err) => {
    if (err) {
      console.error('Erro ao inserir sala:', err);
      return res.status(500).send('Erro ao inserir sala');
    }
...
app.put('/salas/:id', authenticateToken, autenticarAdmin, (req, res) => {
    const { id } = req.params;
...
app.put('/salas/:id/desativar', authenticateToken, autenticarAdmin, (req, res) => {
    const { id } = req.params;
...
app.put('/salas/:id/ativar', authenticateToken, autenticarAdmin, (req, res) => {
    const { id } = req.params;
...
app.get('/salas/ativas', authenticateToken, (req, res) => {
...
app.get('/salas/inativas', authenticateToken, (req, res) => {
